...
namespace CarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminPricingController : Controller
    {
        ...
        [Route("AdminPricingIndex")]
        public async Task<IActionResult> AdminPricingIndex()
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7203/api/Pricings");
        ...
        public async Task<IActionResult> CreatePricing(CreatePricingDto createPricingDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7203/api/Pricings", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("AdminPricingIndex", "AdminPricing", new { area = "Admin" });
            }
            return View();
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7203/api/Pricings?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("AdminPricingIndex", "AdminPricing", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var resposenMessage = await client.GetAsync($"https://localhost:7203/api/Pricings/{id}");
        ...
        public async Task<IActionResult> UpdatePricing(UpdatePricingDto updatePricingDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7203/api/Pricings/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("AdminPricingIndex", "AdminPricing", new { area = "Admin" });
            }
