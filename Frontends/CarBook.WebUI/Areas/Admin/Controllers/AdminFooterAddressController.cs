...
namespace CarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminFooterAddressController : Controller
    {
        ...
        [Route("AdminFooterAddressIndex")]
        public async Task<IActionResult> AdminFooterAddressIndex()
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7203/api/FooterAddress");
        ...
        public async Task<IActionResult> CreateFooterAddress(CreateFooterAddressDto createFooterAddressDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7203/api/FooterAddress", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("AdminFooterAddressIndex", "AdminFooterAddress", new { area = "Admin" });
            }
            return View();
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7203/api/FooterAddress?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("AdminFooterAddressIndex", "AdminFooterAddress", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var resposenMessage = await client.GetAsync($"https://localhost:7203/api/FooterAddress/{id}");
        ...
        public async Task<IActionResult> UpdateFooterAddress(UpdateFooterAddressDto updateFooterAddressDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7203/api/FooterAddress/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("AdminFooterAddressIndex", "AdminFooterAddress", new { area = "Admin" });
            }
